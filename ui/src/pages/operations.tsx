...
import { operationsUiEnabled } from '@/operations/uiGate';

export const getServerSideProps: GetServerSideProps = async () =>
  operationsUiEnabled() ? { props: {} } : { notFound: true };
...
export default function OperationsPage() {
  const operatorUiEnabled = operationsUiEnabled();
  ...
  const loadList = useCallback(async (cursor?: string) => {
    ...
    try {
      setList(await fetchOperations({ pageSize: 25, ...(cursor ? { cursor } : {}) }));
      setListStatus('ready');
    } catch (error) {
      setListStatus('error');
  ...
  if (!operatorUiEnabled) {
    return (
      <>
        <Head>
          <title>Page not found - Game Agent</title>
        </Head>
        <main>
          <h1>Operations console is not enabled.</h1>
        </main>
      </>
    );
  }
              ...
              <>
                <section aria-label="Operations" className="ga-ops-section">
                  ...
                  <OperationsList
                    ...
                    onNext={list?.next_cursor ? () => void loadList(list.next_cursor) : undefined}
