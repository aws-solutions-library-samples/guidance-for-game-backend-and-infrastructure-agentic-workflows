...
describe('KillSwitchPanel', () => {
  ...
  it('disables controls while a submission is in flight (loading)', async () => {
    ...
    await waitFor(() => {
      expect(screen.getByRole('switch', { name: /operations enabled/i })).not.toBeDisabled();
    });
  });

  it('resynchronizes local switches when a newer kill-switch document arrives', async () => {
    const { rerender } = render(
      <KillSwitchPanel killSwitch={killSwitch} capabilities={[capability]} onSubmit={jest.fn()} />,
    );
    await userEvent.click(screen.getByRole('switch', { name: /execute/i }));
    expect(screen.getByRole('switch', { name: /execute/i })).toHaveAttribute('aria-checked', 'true');

    const refreshed: KillSwitchDocument = {
      ...killSwitch,
      config_version: 8,
    ...
    rerender(
      <KillSwitchPanel killSwitch={refreshed} capabilities={[capability]} onSubmit={jest.fn()} />,
    );
    expect(screen.getByRole('switch', { name: /operations enabled/i })).toHaveAttribute(
      'aria-checked',
      'false',
    );
    expect(screen.getByRole('switch', { name: /execute/i })).toHaveAttribute('aria-checked', 'false');
  });
