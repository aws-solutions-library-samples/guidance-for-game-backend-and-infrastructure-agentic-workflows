...
STACK_NAME="${PROJECT_NAME}-kb-cost"
...
echo "=================================================="
echo "💰 Seeding Cost Knowledge Base"
echo "=================================================="
...
DOCS_DIR="$(cd "$(dirname "$0")/../../docs/kb-sources/cost" && pwd)"
if [ ! -d "$DOCS_DIR" ]; then
...
echo "📤 Uploading Cost documentation..."
if ! aws s3 sync "$DOCS_DIR" "s3://$BUCKET_NAME/cost/" \
  --region "$REGION" \
  --exclude ".*"; then
...
while [ $ELAPSED -lt $MAX_WAIT ]; do
  ...
  if [ "$STATUS" = "COMPLETE" ]; then
    echo "✅ Cost KB ingestion complete!"
