...
:func:`~connector.github_provider.get_provider`, and speaks only in the agnostic data
...
from connector.github_provider import get_provider
...
#                            commit_files → open_pull_request, transient-only retries
...
def _build_pr_body(
    ...
    """Compose a PR description referencing every file plus agent+user attribution.
...
def propose_change(
    ...
    *,
        ...
        return _finalize(
            ProposalResult(
                ...
                message=(
                    ...
                    "expressed as a pull request; no proposal was created."
                ),
            ),
            level="info",
    ...
    body = _build_pr_body(description, intent, proposed_files, user_id or "unknown")
    ...
    try:
        ...
        # Open exactly one pull request with agent+user attribution (Req 2.2, 2.6, 6.5).
        pr = _retry_transient(
            lambda: resolved_provider.open_pull_request(
        ...
        # branch was already created but the PR was not, we report failure and NEVER report
        # success (Req 10.3).
        return _finalize(
            _provider_error_result(
    ...
    return _finalize(
        ProposalResult(
            ...
            message=(
                f"Opened change proposal {pr.pull_request_id} against {repo}@{branch}. "
