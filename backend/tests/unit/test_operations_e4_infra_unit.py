...
* Every alarm references a metric constant parsed from the backend emitter
  source, so a hand-written test allowlist cannot drift from the code that runs.
...
# The exact AppConfig control/data actions the control role needs.
...
def test_disable_wrapper_preserves_every_non_mode_parameter(template):
    body = DISABLE_WRAPPER.read_text(encoding="utf-8")
    for name in template["Parameters"]:
        if name == "Provisioned":
            expected = "ParameterKey=Provisioned,ParameterValue=true"
        elif name == "ControlMode":
            expected = "ParameterKey=ControlMode,ParameterValue=disabled"
        else:
            expected = f"ParameterKey={name},UsePreviousValue=true"
        assert expected in body, f"disable wrapper does not preserve {name}"
...
# --------------------------------------------------------------------------- #
# Every E4 alarm MUST reference a metric constant in the backend emitter source.
# Parse the actual source rather than maintaining a second hand-written metric
# inventory that can drift from the deployed code.
def _backend_emitted_operations_metrics() -> frozenset[str]:
    emitted: set[str] = set()
    for source in (PROJECT_ROOT / "backend/src/operations").glob("**/metrics.py"):
        ...
        for node in module.body:
            ...
            if not isinstance(value, ast.Constant) or not isinstance(value.value, str):
                continue
            if any(isinstance(target, ast.Name) and target.id.startswith("METRIC_") for target in targets):
...
def test_all_416_alarms_reference_only_backend_emitted_metrics(template):
    ...
    emitted_metrics = _backend_emitted_operations_metrics()
    # Every alarm's metric must be a real emitted signal -- no fabrication.
    for metric in by_metric:
        assert metric in emitted_metrics, f"alarm on non-emitted metric: {metric}"
