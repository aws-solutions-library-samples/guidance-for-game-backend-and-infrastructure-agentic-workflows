...
PUBLISHED_SCHEMA_FILES = frozenset(f"{name}.schema.json" for name in SCHEMA_NAMES)
EXPECTED_SCHEMA_FILES = frozenset(path.name for path in SCHEMA_SRC_DIR.glob("*.schema.json"))
...
def test_combined_package_contains_all_runtime_schemas(tmp_path):
    ...
    assert PUBLISHED_SCHEMA_FILES.issubset(present)
    assert "gamelift-observation.schema.json" in present

    # Every archived schema, including additive runtime contracts that are not
    # part of the immutable write-contract SCHEMA_NAMES set, must parse as a
    # JSON object whose $id identifies its file stem.
    with zipfile.ZipFile(out) as zf:
        for filename in sorted(EXPECTED_SCHEMA_FILES):
            rel = f"{SCHEMA_REL_DIR}/{filename}"
            ...
            assert doc.get("$id", "").endswith(contract_name), f"{rel} $id must identify {contract_name}"
