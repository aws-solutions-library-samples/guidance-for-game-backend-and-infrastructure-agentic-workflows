...
3. **Provider abstraction layer** (`connector/provider.py`) — `SourceControlProvider` ABC
   defining a fixed operation set; a registry selects the concrete adapter from
   `GBAW_SCM_PROVIDER`.
4. **Provider adapter layer** (for example, `connector/github_provider.py`) — a concrete adapter
   implements every operation against its provider's REST API. Provider-specific types never
   escape this layer. The first adapter targets GitHub; additional adapters (GitLab, CodeCommit)
   plug in without changing the layers above.
...
flowchart TD
    ...
    G7 --> PF[Provider registry -> selected adapter]
    PF --> P1[read latest target-branch HEAD]
    P1 --> P2[create unique proposal branch]
    P2 --> P3[commit modified files]
    P3 --> P4["open change proposal (GitHub PR / GitLab MR) w/ attribution"]
    P4 --> AUD["audit log (redacted)"]
    AUD --> RES[return change proposal id + URL]
    RES --> SCA
```
...
6. **Credential fetch** — `get_secret(config.credential_secret_id, source="secretsmanager")`
   within the provider timeout. Failure or timeout fails closed with no branch/proposal.
...
8. **Provider ops** (transient-only retries, up to `retry_max_attempts`):
   ...
   `create_branch` → `commit_files` → open the change proposal (a GitHub pull request, a GitLab
   merge request, etc., depending on the adapter) with agent + user attribution. A
...
resolution); HTTP 5xx/429 → `ProviderTransientError` (retryable). If change-proposal creation
fails **after** a branch was created, the Connector reports failure and never reports success.
...
provider-agnostic dataclasses; no provider-specific type is referenced in the signatures. The
final operation opens a change proposal, which each adapter realizes as its provider's native
review artifact (GitHub pull request, GitLab merge request, and so on).
...
class SourceControlProvider(ABC):
    ...
    def open_change_proposal(self, repo, head, base, title, body) -> ChangeProposalResult: ...
```

Typed exceptions (`ProviderUnavailableError`, `ProviderAuthError`, `ProviderConflictError`,
`ProviderTransientError`, `UnsupportedProviderError`) let the service layer react uniformly. A
Provider-neutral registry selects the adapter for the configured provider and fails closed when
...
**Adapter details (GitHub example).** The GitHub adapter uses `httpx` (pinned) with a per-request
timeout from `GBAW_SCM_PROVIDER_TIMEOUT_SECONDS`; **no local `git clone`** (the container FS is
read-only) — it uses the GitHub Git Data / Contents REST endpoints and opens a pull request as the
change proposal. The credential is fetched per-operation via `get_secret(...)`, placed in the
`Authorization` header, and never logged. Outbound HTTPS to the provider host (`api.github.com`,
or a configured enterprise base URL) must be allowed from the AgentCore runtime. Other adapters
(GitLab, CodeCommit) follow the same shape against their own APIs and review artifacts.
...
| `GBAW_SCM_PROVIDER_BASE_URL` | — | Optional Provider API base URL (for example, a self-hosted enterprise endpoint) |
...
1. **Provision the write credential.** Create a Secrets Manager secret containing a scoped
   provider token (for GitHub, a token or App installation token) with the permissions needed to
   push a branch and open a change proposal on the target repo. Note its ARN.
...
5. **Allow egress.** Ensure the AgentCore runtime can reach the provider host
   (`api.github.com` or your configured enterprise base URL) over HTTPS.
...
Validation runs **before** any branch/commit/change-proposal call and operates on the
agent-proposed file contents in memory (read-only FS friendly):
...
| 16–20 | Provider unavailability is safe; invalid credentials not retried; proposal-failure-after-branch never reports success; conflicts reported without destructive resolution; transient errors retried up to the configured maximum |
...
│   ├── models.py            # FileContent, FileFetchResult, ProposedFile, ChangeProposalResult, ProposalResult
│   ├── provider.py          # SourceControlProvider ABC + typed exceptions
│   ├── registry.py          # Provider-neutral registry + get_provider selector
│   ├── github_provider.py   # GitHub adapter (registers with the provider registry)
